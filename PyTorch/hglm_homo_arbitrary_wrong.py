...
from h_likelihood_arbitrary import Sigma_module, nhll_homo_arbitrary
...
def homo_arbitrary_without_val(
    ...
    Sigma   = Sigma_module(p, K, device=device).to(device)
    ...
    for iter in tqdm(range(pretrain_iter)) : 
        ...
        with torch.no_grad() : 
            ...
            Sigma_v = torch.bmm(L, L.transpose(1,2))
    ...
    for iter in tqdm(range(max_iter)) : 
        ...
        for epoch in range(max_epoch) : 
            for _, (image, hp, y, batch_ids) in enumerate(train_loader) : 
                ...
                mean_optimizer.zero_grad()
                variance_optimizer.zero_grad()
                train_loss = nhll_homo_arbitrary(train_N, y, batch_fixed, batch_random, v_list, log_phi, Sigma, weighted=weighted, n_list=train_n_list, batch_n_list=batch_n_list, update='M', verbose=verbose)
                train_loss.backward()
                mean_optimizer.step()
        ...
        if nhll_update_stop is False and train_nhll < best_nhll : 
        ...
        # L_list[pretrain_iter + iter] = L.cpu().numpy()
    ...
    np.save(f'./Prediction/homo_arbit_{experiment_name}_L', L_list)
    # np.save(f'./Prediction/homo_arbit_{experiment_name}_log_lamb', log_lamb_list)
