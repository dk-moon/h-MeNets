...
from likelihood_hetero import multivariate_nll_hetero_i, multivariate_njll_hetero_i
from h_likelihood_arbitrary import Sigma_module_rev, nhll_hetero_arbitrary
from util import make_reproducibility, TensorDataset, convert_to_spherical, convert_to_xyz, mae

def hetero_arbitrary_without_val(
    train_ids, train_images, train_hps, train_gazes, 
    test_ids, test_images, test_hps, test_gazes, 
    mean_network, hidden_features, K=2, initialize_Sigma=False,
    ...
    torch.save(mean_model.state_dict(), f'./Model/hetero_arbit{experiment_name}_pretrained_mean_model.pt')
    torch.save(log_phi_layer.state_dict(), f'./Model/hetero_arbit{experiment_name}_pretrained_variance_model.pt')
    torch.save(Sigma.state_dict(), f'./Model/hetero_arbit{experiment_name}_pretrained_Sigma.pt')
    ...
    np.save(f'./Prediction/hetero_arbit_{experiment_name}_pred', prediction)
    np.save(f'./Prediction/hetero_arbit_{experiment_name}_pred_adjusted', prediction_adjusted)
    np.save(f'./Prediction/hetero_arbit_{experiment_name}_train_loss', train_loss_list)
    np.save(f'./Prediction/hetero_arbit_{experiment_name}_test_loss', test_loss_list)
    np.save(f'./Prediction/hetero_arbit_{experiment_name}_v_list', v_list_list)
    np.save(f'./Prediction/hetero_arbit_{experiment_name}_Sigma', Sigma_list)
    np.save(f'./Prediction/hetero_arbit_{experiment_name}_beta', beta_list)
    np.save(f'./Prediction/hetero_arbit_{experiment_name}_w', w_list)
    np.save(f'./Prediction/hetero_arbit_{experiment_name}_train_Gamma', train_Gamma_list_list)
    np.save(f'./Prediction/hetero_arbit{experiment_name}_test_Gamma', test_Gamma_list_list)

    torch.save(mean_model.state_dict(), f'./Model/hetero_arbit_{experiment_name}_trained_mean_model.pt')
    torch.save(best_nhll_mean_model.state_dict(), f'./Model/hetero_arbit_{experiment_name}_nhll_selected_mean_model.pt')
    
    torch.save(log_phi_layer.state_dict(), f'./Model/hetero_arbit_{experiment_name}_trained_variance_model.pt')
    torch.save(best_nhll_log_phi_layer.state_dict(), f'./Model/hetero_arbit_{experiment_name}_nhll_selected_variance_model.pt')
    
    torch.save(Sigma.state_dict(), f'./Model/hetero_arbit_{experiment_name}_trained_Sigma.pt')
    torch.save(best_nhll_Sigma.state_dict(), f'./Model/hetero_arbit_{experiment_name}_nhll_selected_Sigma.pt')
