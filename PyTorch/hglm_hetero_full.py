...
def hetero_diag_without_val(
    ...
    variance_optimizer = optim.Adam(list(log_phi_layer.parameters()) + [log_lamb], lr=variance_lr, weight_decay=weight_decay)
    ...
    for iter in tqdm(range(pretrain_iter)) : 
        ...
        for epoch in range(m_pretrain_epoch) : 
            for _, (image, hp, y, batch_ids) in enumerate(train_loader) : 
                ...
                pretrain_loss = nhll_hetero_diag(train_N, y, batch_fixed, batch_random, v_list, weighted=weighted, n_list=train_n_list, batch_n_list=batch_n_list, verbose=verbose, update='pretrain')
    ...
    torch.save(mean_model.state_dict(), f'./Model/hetero_{experiment_name}_pretrained_mean_model.pt')
    torch.save(log_phi_layer.state_dict(), f'./Model/hetero_{experiment_name}_pretrained_variance_model.pt')
    ...
    for iter in tqdm(range(max_iter)) : 
        ...
        for epoch in range(mean_epoch) : 
            ...
            for _, (image, hp, y, batch_ids) in enumerate(train_loader) : 
                ...
                mean_optimizer.zero_grad()
                variance_optimizer.zero_grad()
                train_loss = nhll_hetero_diag(train_N, y, batch_fixed, batch_random, v_list, batch_log_phi, log_lamb, weighted=weighted, n_list=train_n_list, batch_n_list=batch_n_list, update='M', verbose=verbose)
                train_loss.backward()
                mean_optimizer.step()
            ...
            with torch.no_grad() : 
                ...
                mean_optimizer.zero_grad()
                variance_optimizer.zero_grad()
                train_loss = nhll_hetero_diag(train_N, train_y_cuda, train_fixed, train_random, v_list, train_log_phi, log_lamb, 
                                            train_Gamma_list, train_log_phi_list, verbose=verbose, update='V-full')
        ...
        # print(f'{epoch}-th epoch last batch Pretrain h-lik loss (v-step) : {train_loss.item()}')
            ...
            #     mean_optimizer.zero_grad()
            #     variance_optimizer.zero_grad()
            #     train_loss = nhll_hetero_diag(train_N, train_y[train_cluster[i]].to(device), train_fixed[train_cluster[i]], train_random[train_cluster[i]], 
            #                                   v_list, batch_log_phi, log_lamb, 
        ...
        with torch.no_grad() : 
            ...
            train_nhll = sum([multivariate_nhll_hetero_i(train_y_list[i], train_fixed_list[i], train_Gamma_list[i], v_list[i], torch.exp(train_log_phi_list[i]), Sigma_v) for i in range(m)]).item() / train_N
    ...
    np.save(f'./Prediction/hetero_{experiment_name}_homo_pred', prediction)
    np.save(f'./Prediction/hetero_{experiment_name}_homo_pred_adjusted', prediction_adjusted)
    np.save(f'./Prediction/hetero_{experiment_name}_train_loss', train_loss_list)
    np.save(f'./Prediction/hetero_{experiment_name}_test_loss', test_loss_list)
    np.save(f'./Prediction/hetero_{experiment_name}_v_list', v_list_list)
    np.save(f'./Prediction/hetero_{experiment_name}_log_lamb', log_lamb_list)
    np.save(f'./Prediction/hetero_{experiment_name}_beta', beta_list)
    np.save(f'./Prediction/hetero_{experiment_name}_w', w_list)
    np.save(f'./Prediction/hetero_{experiment_name}_train_Gamma', train_Gamma_list_list)
    np.save(f'./Prediction/hetero_{experiment_name}_test_Gamma', test_Gamma_list_list)

    torch.save(mean_model.state_dict(), f'./Model/hetero_{experiment_name}_trained_mean_model.pt')
    torch.save(best_nhll_mean_model.state_dict(), f'./Model/hetero_{experiment_name}_nhll_selected_mean_model.pt')

    torch.save(log_phi_layer.state_dict(), f'./Model/hetero_{experiment_name}_trained_variance_model.pt')
    torch.save(best_nhll_log_phi_layer.state_dict(), f'./Model/hetero_{experiment_name}_nhll_selected_variance_model.pt')
